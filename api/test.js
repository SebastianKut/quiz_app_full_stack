...
router.get('/:set', (req, res) => { 
 ...
 router.post('/submittest', (req, res) => {
    ...
    const sql = `INSERT INTO users (first_name, last_name) VALUES (?,?);
                 ...
                 INSERT INTO answers (questions_id, choices_id, tests_id, users_id) VALUES 
                        ((SELECT question_id FROM questions WHERE question_id = ?),
                        (SELECT choice_id FROM question_choices WHERE choice_id = ?),
                        LAST_INSERT_ID(),
                        (SELECT users_id FROM tests WHERE test_id = LAST_INSERT_ID())),
                        ((SELECT question_id FROM questions WHERE question_id = ?),
                        (SELECT choice_id FROM question_choices WHERE choice_id = ?),
                        LAST_INSERT_ID(),
                        (SELECT users_id FROM tests WHERE test_id = LAST_INSERT_ID())),
    ...
    db.query(sql, data, (err, result) => {
        if (err) throw err;
        const testId = result[1].insertId; //result of querry is array of objects - number of objects represents no of querries, each querry has insertId which can be used to return testId generated by database then used to get results
 ...
 router.get('/getresults/:testid', (req, res) => { 
    ...
    FROM answers
    INNER JOIN question_choices on question_choices.choice_id = answers.choices_id
    ...
    db.query(sql, (err, result) => {
        ...
        if (!found) { 
            ...
            testResult = correct/answers.length;
